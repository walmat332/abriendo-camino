...
import { ArrowRight, ArrowLeft, Compass, Sprout, Send, Check } from 'lucide-react'
...
const fases = [
  {
    ...
    pregunta: '¿Dónde sientes que Dios te está invitando a dar un paso de fe?',
    opciones: [
  ...
  },
  {
    ...
    pregunta: '¿Qué capacidad, experiencia o don puedes usar para bendecir a otros?',
    opciones: [
  ...
  },
  {
    ...
    pregunta: '¿Con quién puedes compartir lo que Dios está haciendo en tu vida?',
    opciones: [
      'Un amigo cercano',
      'Un familiar',
      'Un compañero de trabajo o estudio',
      'Alguien de mi comunidad',
      'Todavía no tengo a nadie en mente',
    ],
  },
]
...
export function PropositoFlow({ onComplete, onBack }: PropositoFlowProps) {
  ...
  const getBtnColor = () => {
    if (!seleccion) return '#e2e8f0'
    if (faseActual === 0) return '#3b82f6'
    if (faseActual === 1) return '#10b981'
    return '#f59e0b'
  }
            ...
            {fase.opciones.map((opcion, idx) => (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <span className="flex-1 text-slate-700 font-medium group-hover:text-slate-900">
                    {opcion}
                  </span>
                </div>
              </button>
            ...
            {fases.map((_, idx) => (
              <div
                ...
                className={`h-1.5 rounded-full transition-all duration-500 ${
                  idx === faseActual ? 'bg-slate-800 w-8' : idx < faseActual ? 'bg-slate-400 w-4' : 'bg-slate-200 w-4'
                }`}
              />
            ))}
          </div>
